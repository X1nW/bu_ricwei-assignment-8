...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    X2[:, 0] += distance  # Shift along the x-axis
    X2[:, 1] -= distance  # Shift along the y-axis
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.plot(np.linspace(x_min, x_max, 100), slope * np.linspace(x_min, x_max, 100) + intercept, 'k--', label='Decision Boundary', linewidth=2)
